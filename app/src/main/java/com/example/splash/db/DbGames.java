...
public class DbGames extends DbHelper {
    ...
    public long insertaRecordLt(boolean win, int moves, String player) {
        long id = 0;
        try {
    ...
    public long insertaRecordTf(boolean win, int moves, String player) {
        long id = 0;
        try {
            DbHelper dbHelper = new DbHelper(mContext);
            SQLiteDatabase db = dbHelper.getWritableDatabase();
    ...
    public long insertaUser(String username) {
        long id = 0;
        try {
            DbHelper dbHelper = new DbHelper(mContext);
            SQLiteDatabase db = dbHelper.getWritableDatabase();
            ...
            id = db.insert(UTABLE, null, values);
        } catch (Exception e) {
            Log.println(Log.ASSERT, "error", "no se ha podido conectar a la tabla");
        }
    ...
    public List<String> getAllUsers() {
        String selectQuery = "SELECT * FROM " + UTABLE;
        List<String> list = new ArrayList<String>();
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor cursor = db.rawQuery(selectQuery, null);//selectQuery,selectedArguments
        ...
        try {
            ...
            if (cursor.moveToFirst()) {
                do {
                    list.add(cursor.getString(1));//adding 0nd column data
                } while (cursor.moveToNext());
            }
        } catch (Exception e) {
